...
@section('content')
    ...
    <div class="panel panel-inverse">
        ...
        <!-- end panel-heading -->
        <form action="{{ route('admin.regulation.update', ['regulation' => $regulation['id']]) }}" enctype="multipart/form-data" method="POST">
            @csrf
                            ...
                            <!-- begin tab-pane -->
                                <div class="tab-pane fade{{(!$loop->index)?' active show':''}}"
                                    ...
                                    <input type="text" name="title[{{$lang['url']}}]" id="title[{{$lang['url']}}]"
                                           ...
                                           value="{{ $regulation['title'][$lang['url']] }}">
...
{{--                        <div class="col-xl-9">--}}
                            ...
                            <select name="regulation_category_id" id="regulation_category_id" class="form-control">
                                @foreach($categories as $category)
                                    <option value="{{$category->id}}" @if($regulation['regulation_category_id']==$category->id) selected @endif>{{$category->name}}</option>
                                @endforeach
                            </select>
                        ...
                        <div class="row m-10">
                            ...
                            <div class="col-xl-4">
                                ...
                                <input type="date" name="published_date" id="published_date" class="form-control"
                                       value="{{ $regulation['published_date'] }}">
                            ...
                            <div class="col-xl-4">
                                ...
                                <div class="switcher">
                                    <input type="checkbox" name="status" id="status" {{ $regulation['status']?'checked':'' }} value="1">
                                ...
                                <table class="table table-striped table-condensed text-nowrap mb-0">
                                    ...
                                    <tr data-id="empty">
                                        <td colspan="4" class="text-center text-muted p-t-30 p-b-30">
                                            ...
                                            <div>{{ substr($regulation['document'], strpos($regulation['document'], '_', 19)+1) }}</div>
                                            <div><a href="{{ asset($regulation['document']) }}" download=""><i class="fa fa-download"></i> Скачать</a></div>
                                        </td>
                                    </tr>
