...
@section('content')
    ...
    <ol class="breadcrumb float-xl-right">
        ...
        <li class="breadcrumb-item"><a href="{{route('admin.article.index')}}">Новости</a></li>
    ...
    <div class="panel panel-inverse" style="background: #f4f4f7">
        ...
        <!-- end panel-heading -->
        <form action="{{ route('admin.article.update', ['article' => $article['id']]) }}" enctype="multipart/form-data" method="POST">
            @csrf
            <div class="panel-body p-t-10">
                    ...
                    <div class="col-xl-8">
                            ...
                            <!-- begin tab-pane -->
                                <div class="tab-pane fade{{(!$loop->index)?' active show':''}}"
                                    ...
                                    <textarea class="ckeditor" id="editor{{$lang['url']}}"
                                              ...
                                              rows="20">{{ $article['description'][$lang->url] }}</textarea>
                        ...
                        <div class="row">
                                ...
                                <select name="article_category_id" id="article_category_id" class="form-control">
                                    @foreach($categories as $category)
                                        <option value="{{$category->id}}"
                                                @if($article['article_category_id']==$category->id) selected @endif>{{$category->name}}</option>
                                    @endforeach
                                </select>
                            ...
                            <div class="col-xl-5">
                                ...
                                <input type="datetime-local" name="published_date" id="published_date" class="form-control"
                                       value="{{ date('Y-m-d\TH:i', strtotime($article['published_date'])) }}">
                            ...
                            <div class="col-xl-1">
                                ...
                                <div class="switcher">
                                    <input type="checkbox" name="status" id="status"
                                           {{ $article['status']?'checked':'' }} value="1">
                    ...
                    <div class="col-xl-4">
                        <div class="row">
                            ...
                            <div class="col-xl-7 mt-3">
                                <img id="image-src" width="100%" src="{{ $article['image'] }}">
                            </div>
                            <!-- end col-6 -->
                        </div>
                        <hr>
                        <div class="row">
                            ...
                            <div class="col-xl-7 mt-3">
                                <img id="cover_image-src" width="100%" src="{{ $article['cover_image'] }}">
                            </div>
                            <!-- end col-6 -->
                        </div>
                    </div>
                ...
                <div class="superbox" data-offset="50">
                    @foreach($images as $image)
                        <div class="superbox-list" id="image{{$image['id']}}">
                            <a href="javascript:;" class="superbox-img">
                                <img data-img="{{ asset($image->image) }}" />
                                <span style="background: url({{ asset($image->image) }});">
{{--                                    <span class="badge bg-danger" style="width: 25px; height: 25px">x</span>--}}
                                    <div class="checkbox checkbox-css is-invalid">
                                        <input type="checkbox" id="cssCheckbox{{ $image['id'] }}" name="remove_images[]" value="{{ $image['id'] }}">
                                        <label for="cssCheckbox{{ $image['id'] }}">Удалить</label>
								    </div>
                                </span>
...
{{--                            <input type="checkbox" id="Checkbox{{$image['id']}}" value="{{$image['id']}}" onchange="console.log('ds')" />--}}
{{--                            <label for="Checkbox{{$image['id']}}">Удалить</label>--}}
{{--                        </div>--}}
                        </div>
                    @endforeach
                </div>
